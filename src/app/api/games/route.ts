...
export async function GET() {
  try {
    ...
    const jogos = ((data.matches as Match[]) || [])
      ...
      .slice(0, 4)
      .map((match: Match) => ({
        hora_realizacao: new Date(match.utcDate).toLocaleTimeString('pt-BR', {
          hour: '2-digit',
          minute: '2-digit',
        ...
        time_mandante: {
          ...
          gols: match.status === 'FINISHED' ? match.score.fullTime.home : null,
        },
        time_visitante: {
          sigla:
            match.awayTeam.tla ||
          ...
          gols: match.status === 'FINISHED' ? match.score.fullTime.away : null,
        },
